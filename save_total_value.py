...
async def config_list_empresas(list_empresas):
    ...
    for line in list_empresas.itertuples():
        ...
        value_total = getattr(line, '_14')
        if not pd.isna(value_total) and value_total != "nan" and value_total != "0,00" and value_total != "0":
            value_total = str(value_total).replace(' ', '').replace(',', '.')
            value_total = value_total.replace('.', '', value_total.count('.') - 1)

            try: 
                value_total = float(value_total)
                value_total = f"{value_total:,.2f}".replace(",", "_").replace(".", ",").replace("_", ".")
                if value_total != '0,00':
                    item = {
                        "name": line.name,
...
async def register_odoo(list_empresas):
    async with async_playwright() as p:
        ...
        for empresa in list_empresas[2:]:
            print(empresa)
            if empresa['name'] not in company_igonre:
                ...
                await page.locator("//html/body/div[1]/div/div/div[2]/div/div[1]/div[2]/div[5]/div[1]/ul/li[2]/a").click()
                time.sleep(1)

                # input Total
                input_total_value = page.locator("//html/body/div[1]/div/div/div[2]/div/div[1]/div[2]/div[5]/div[2]/div/div/div[1]/div[4]/div[2]/div/div/input")
                await input_total_value.fill(empresa['value_total'])
                await input_total_value.press('Enter')
